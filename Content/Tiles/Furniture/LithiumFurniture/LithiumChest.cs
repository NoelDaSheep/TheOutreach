...
{
	public class LithiumChest : ModTile
	{
		public override void SetStaticDefaults() {
			...
			ChestDrop = ModContent.ItemType<Items.Placeable.Furniture.LithiumChest>();

			// Names
			ContainerName.SetDefault("Lithium Chest");

			ModTranslation name = CreateMapEntryName();
			name.SetDefault("Lithium Chest");
			AddMapEntry(new Color(200, 200, 200), name, MapChestName);

			name = CreateMapEntryName(Name + "_Locked"); // With multiple map entries, you need unique translation keys.
			name.SetDefault("Locked Lithium Chest");
		...
		public override void MouseOver(int i, int j)
		{
			...
			{
				player.cursorItemIconText = Main.chest[chest].name.Length > 0 ? Main.chest[chest].name : "Lithium Chest";
				if (player.cursorItemIconText == "Example Chest")
				{
					player.cursorItemIconID = ModContent.ItemType < Items.Placeable.Furniture.LithiumChest>();
