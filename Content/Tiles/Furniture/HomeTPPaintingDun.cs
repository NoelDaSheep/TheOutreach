...
{
	public class HomeTPPaintingDun : ModTile
	{
		...
		{
			Item.NewItem(new EntitySource_TileBreak(i, j), i * 16, j * 16, 32, 32, ModContent.ItemType<Items.Placeable.Furniture.HomeTPPaintingDun>());
		...
		public override bool RightClick(int i, int j)
		{
			...
			for (int x = 0; x < Main.maxTilesX; ++x) // LOOP WORLD X
			{
				for (int y = 0; y < Main.maxTilesY; ++y) // LOOP WORLD Y
				{
					...
					if (Main.tile[x, y].TileType != ModContent.TileType<DungeonTPPainting>()) continue;
