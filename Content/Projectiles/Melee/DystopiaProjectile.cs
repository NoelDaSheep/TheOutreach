...
namespace TheOutreach.Content.Projectiles.Melee
{
	public class DystopiaProjectile : ModProjectile
	{
		...
		public bool hasUsedSpecial = false;

		Vector2 velocityThreshhold;

		public override void SetStaticDefaults() {
			DisplayName.SetDefault("Dytopian Sai");
		}
        ...
        /*public override bool PreDraw(ref Color lightColor)
        {
			...
			for (int i = 0; i < 1000; i++)
			{
				if (distance > 4f && !float.IsNaN(distance))
				{
					...
					//Draw chain
					Main.EntitySpriteDraw(Request<Texture2D>("TheOutreach/Content/Projectiles/Melee/DystopiaChain").Value, new Vector2(center.X - Main.screenPosition.X, center.Y - Main.screenPosition.Y),
        ...
        public override void AI()
        {
            ...
            if (HasHit && !hasUsedSpecial)
            {
				Player player = Main.player[Projectile.owner];
				if (TheOutreachMod.WeaponSpecialKeybind.JustPressed)
				{
					player.velocity = Projectile.position - player.position;
					hasUsedSpecial = true;
				}
			}
		}
